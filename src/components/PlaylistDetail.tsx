...
function PlaylistDetailSkeleton({ pageBgColor, solidOnLight }: { pageBgColor?: string | null; solidOnLight?: boolean }) {
      ...
      >
        ...
        >
          ...
          <div className="flex-1 min-w-0 text-center md:text-left w-full">
            <div className={`h-2 w-20 rounded-full ${heroPulseAlt} mx-auto md:mx-0`} />
        ...
        <div className={`overflow-hidden rounded-none md:rounded-2xl border-y md:border -mx-5 md:mx-0 divide-y ${tableCls}`}>
          {titleWidths.map((w, i) => (
            <div key={i} className="grid grid-cols-[2.5rem,1fr,2.5rem] items-center gap-2 md:gap-3 px-3 md:px-4 py-[9px]">
              <div className={`w-9 h-9 rounded-lg flex-shrink-0 ${rowPulse}`} />
              <div className="flex flex-col gap-1.5 min-w-0">
                <div className={`h-[14px] ${w} rounded-full ${rowPulse}`} />
                <div className={`h-[11px] ${subtitleWidths[i]} rounded-full ${rowPulseAlt}`} />
              </div>
...
export default function PlaylistDetail() {
  ...
  // Use activeCoverUrl directly: it's available at init time and likely already cached
  // from the playlist card that was just clicked, so colors resolve without waiting
  // for PageHero's onImageLoaded callback.
  ...
  return (
    <div ref={rootRef} style={pageBgColor ? { ['--bg-base' as string]: pageBgColor } : undefined}>
      {loading && !pinnedFallback ? (
        <PlaylistDetailSkeleton pageBgColor={pageBgColor} solidOnLight={solidOnLight} />
      ) : (
        <>
          <PageHero
            type="playlist"
